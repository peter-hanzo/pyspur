...
Finally, we need to combine both routes and send the results to a Slack channel named `learning`.
...
- **Keyboard shortcuts:**
  - `⌘/CTRL + ENTER` for running the workflow on the most recent test case.
  - `⌘/CTRL + I` for re-ordering the layout.
- **Jinja Templates**
  - Under the hood, we use [Jinja](https://jinja.palletsprojects.com/en/stable/) to create strings (e.g., messages) with data from previous nodes.
