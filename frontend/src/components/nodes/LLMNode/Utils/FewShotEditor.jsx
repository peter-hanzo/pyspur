...
const FewShotEditor = ({ nodeID, exampleIndex, onSave, onDiscard }) => {
    ...
    const [inputContent, setInputContent] = useState(node?.data?.fewShotExamples?.[exampleIndex]?.input || '');
    const [outputContent, setOutputContent] = useState(node?.data?.fewShotExamples?.[exampleIndex]?.output || '');
    ...
    useEffect(() => {
        if (inputEditor) {
            inputEditor.commands.setContent(node?.data?.fewShotExamples?.[exampleIndex]?.input || '');
        }
        if (outputEditor) {
            outputEditor.commands.setContent(node?.data?.fewShotExamples?.[exampleIndex]?.output || '');
        }
