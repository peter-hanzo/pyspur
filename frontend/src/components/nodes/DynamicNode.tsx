...
const DynamicNode: React.FC<DynamicNodeProps> = ({
  ...
  const inputMetadata = useSelector(
    (state: RootState) => selectPropertyMetadata(state, `${type}.input`),
    isEqual,
  );
  const outputMetadata = useSelector(
    (state: RootState) => selectPropertyMetadata(state, `${type}.output`),
    isEqual,
  );

  const excludeSchemaKeywords = (
  ...
  useEffect(() => {
    ...
    const outputLabels = nodeData?.title
    ...
    nodeData,
    cleanedInputMetadata,
    cleanedOutputMetadata,
    predecessorNodes,
    nodeWidth,
  ]);
  ...
  const InputHandleRow: React.FC<HandleRowProps> = ({ id, keyName }) => {
    ...
    const isConnectable =
      !isCollapsed &&
      (connections.length === 0 || String(keyName).startsWith("branch"));
    ...
    return (
      ...
      >
        <div
          className={`${styles.handleCell} ${styles.inputHandleCell}`}
          id={`input-${keyName}-handle`}
        >
          <Handle
            type="target"
  ...
  const OutputHandleRow: React.FC<HandleRowProps> = ({ keyName }) => {
    return (
      ...
      >
        ...
        <div
          className={`${styles.handleCell} ${styles.outputHandleCell}`}
          id={`output-${keyName}-handle`}
        >
          <Handle
            type="source"
  ...
  const finalPredecessors = useMemo(() => {
    ...
    if (
      ...
      if (
        connection.fromNode &&
        !updatedPredecessorNodes.find(
          (node: any) => node.id === connection.fromNode.id,
        )
      ) {
        if (
          ...
          result = [
            ...updatedPredecessorNodes,
            {
              ...
              data: {
                title:
                  (connection.fromNode.data as { title?: string })?.title ||
  ...
  useEffect(() => {
    ...
    const hasChanged =
      ...
      finalPredecessors.some(
        (newNode, i) => !isEqual(newNode, predecessorNodes[i]),
      );
  ...
  const renderHandles = () => {
    ...
    return (
      <div className={`${styles.handlesWrapper}`} id="handles">
        {/* Input Handles */}
        <div
          className={`${styles.handlesColumn} ${styles.inputHandlesColumn}`}
          id="input-handles"
        >
        ...
        <div
          className={`${styles.handlesColumn} ${styles.outputHandlesColumn}`}
          id="output-handle"
        >
          {nodeData?.title && (
            <OutputHandleRow id={id} keyName={String(nodeData?.title)} />
          )}
        </div>
      </div>
    );
