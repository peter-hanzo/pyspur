...
const CollapsibleNodePanel: React.FC = () => {
  const isExpanded = useSelector(
    (state: RootState) => state.panel.isNodePanelExpanded,
  );
  const dispatch = useDispatch();
  const nodes = useSelector((state: RootState) => state.flow.nodes);
  const nodeTypes = useSelector(
    (state: RootState) =>
      state.nodeTypes.data as FlowWorkflowNodeTypesByCategory,
  );
  ...
  const [filteredNodeTypes, setFilteredNodeTypes] =
    useState<FlowWorkflowNodeTypesByCategory>({});
  ...
  return (
    ...
    >
      ...
      >
        <Icon
          icon={
            isExpanded ? "solar:minus-square-linear" : "solar:widget-add-linear"
          }
          width={"80%"}
      ...
      {isExpanded && (
        <>
          <Input
            ...
            startContent={
              <Icon icon="akar-icons:search" className="text-default-500" />
            }
          />
          <div
            className="mt-4 max-h-[calc(100vh-16rem)] overflow-auto"
            id="node-type-accordion"
          >
            <Accordion
              selectionMode="multiple"
              selectedKeys={Array.from(selectedCategory)}
              onSelectionChange={(keys) =>
                setSelectedCategory(new Set(Array.from(keys) as string[]))
              }
            >
              {Object.keys(filteredNodeTypes)
                ...
                .map((category) => (
                  <AccordionItem key={category} title={category}>
                    {filteredNodeTypes[category].map(
                      (node: FlowWorkflowNodeType) => (
                        <div
                          key={node.name}
