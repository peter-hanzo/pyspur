...
function rebuildRouterNodeSchema(
  state: FlowState,
  routerNode: FlowWorkflowNode,
) {
  const incomingEdges = state.edges.filter(
    (edge) => edge.target === routerNode.id,
  );
  ...
  const newOutputSchema = incomingEdges.reduce(
    (schema, edge) => {
      ...
      const sourceNodeConfig = sourceNode
        ? state.nodeConfigs[sourceNode.id]
        : undefined;
...
function rebuildCoalesceNodeSchema(
  state: FlowState,
  coalesceNode: FlowWorkflowNode,
) {
  const incomingEdges = state.edges.filter(
  ...
  const schemas: Record<string, any>[] = incomingEdges.map((ed) => {
    ...
    return sourceNode
      ? state.nodeConfigs[sourceNode.id]?.output_schema || {}
      : {};
  });
  ...
  reducers: {
    initializeFlow: (
      ...
      let edges = links.map((link) => ({
        ...
        sourceHandle:
          link.source_handle ||
          state.nodes.find((node) => node.id === link.source_id)?.data?.title,
        targetHandle:
          link.target_handle ||
          state.nodes.find((node) => node.id === link.source_id)?.data?.title,
      }));
    ...
    setSelectedNode: (
      state,
      action: PayloadAction<{ nodeId: string | null }>,
    ) => {
    ...
    deleteEdge: (state, action: PayloadAction<{ edgeId: string }>) => {
      ...
      if (edge) {
        ...
        const targetNodeConfig = targetNode
        ...
        if (
          targetNode?.type === "RouterNode" &&
          sourceNodeConfig?.output_schema
        ) {
        ...
        if (targetNode?.type === "CoalesceNode") {
          ...
          const incomingEdges = state.edges.filter(
            (e) => e.target === targetNode.id && e.id !== edgeId,
          );
          ...
          if (schemas.length > 0) {
            ...
            const commonKeys = Object.keys(firstSchema).filter((key) =>
              schemas.every(
                (sch) =>
                  sch.hasOwnProperty(key) && sch[key] === firstSchema[key],
              ),
            );
            commonKeys.forEach((key) => {
              intersection[key] = firstSchema[key];
      ...
      if (inputNode?.id) {
        ...
        const connectedRouterNodes = state.nodes.filter(
          (targetNode) =>
            ...
            state.edges.some(
              (edge) =>
                edge.source === inputNode.id && edge.target === targetNode.id,
            ),
        );
        connectedRouterNodes.forEach((routerNode) => {
          rebuildRouterNodeSchema(state, routerNode);
    ...
    updateWorkflowInputVariableKey: (
      ...
      state.workflowInputVariables[newKey] =
        state.workflowInputVariables[oldKey];
    ...
    resetFlow: (
      state,
      action: PayloadAction<{ definition: WorkflowDefinition }>,
    ) => {
      ...
      state.edges = links.map((link) => ({
        ...
        targetHandle:
          link.target_handle ||
          state.nodes.find((node) => node.id === link.target_id)?.data?.title,
      }));
    },
    ...
    updateNodeTitle: (
      ...
      const oldTitle = state.nodes.find((node) => node.id === nodeId)?.data
        ?.title;
      ...
      state.edges = state.edges.map((edge) => {
        ...
        if (isRouterNode && edge.source === nodeId) {
          ...
          updatedEdge.targetHandle = edge.targetHandle.replace(
            oldTitle,
            newTitle,
          );
        }
