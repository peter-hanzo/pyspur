...
export function rolloutWorkflowDefinition({
  workflowDefinition,
  tasks,
}: RolloutWorkflowParams): {
  rolledOutDefinition: WorkflowDefinition;
  outputs: Record<string, any>;
} {
  // Create a deep copy of the workflow definition to avoid mutations
  const rolledOutDefinition: WorkflowDefinition = JSON.parse(
  ...
  tasks.forEach((task) => {
    ...
    const nodeToReplace = rolledOutDefinition.nodes.find(
      (node) => node.id === task.node_id,
    );
    ...
    const inputNode = subworkflowNodes.find(
      (node) => node.node_type === "InputNode",
    );
    const outputNode = subworkflowNodes.find(
