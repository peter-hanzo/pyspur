...
type BufferedEdge = FlowWorkflowEdge & { selected?: boolean }
...
export function useCopyPaste() {
    ...
    const paste = useCallback(() => {
        ...
        const updatedNodes = [...nodes.map((node) => ({ ...node, selected: false })), ...newNodes]

        const updatedEdges = [...edges.map((edge) => ({ ...edge, selected: false })), ...newEdges]
