...
class MongoDBAtlasDataStore(DataStore):
    ...
    async def _upsert(self, chunks: Dict[str, List[DocumentChunk]]) -> List[str]:
        ...
        logger.info(
            f"Upsert documents into MongoDB collection: {self.database_name}: {self.collection_name}"
        )
        await self.client[self.database_name][self.collection_name].bulk_write(
            documents_to_upsert
        )
    ...
    async def _execute_embedding_query(self, query: QueryWithEmbedding) -> QueryResult:
        ...
        pipeline = [
            {
                "$vectorSearch": {
                    ...
                    "numCandidates": min(
                        query.top_k * self.oversampling_factor, MAX_CANDIDATES
                    ),
                    "limit": query.top_k,
                }
            },
        ...
        async with self.client[self.database_name][self.collection_name].aggregate(
            pipeline
        ) as cursor:
            results = [
    ...
    async def delete(
        ...
        # Do nothing
            ...
            logger.warning(
                "No criteria set; nothing to delete args: ids: %s, filter: %s delete_all: %s",
                ids,
                filter,
                delete_all,
            )
        ...
        try:
            await self.client[self.database_name][self.collection_name].delete_many(
                mg_filter
            )
            logger.info("Deleted documents successfully")
        except Exception as e:
            logger.error(
                "Error deleting documents with filter: %s -- error: %s", mg_filter, e
            )
            return False
        ...
        for field, value in filter.dict().items():
            if value is not None:
                if field == "start_date":
                    ...
                    mongo_filters["$and"].append({f"metadata.{field}": value})
    ...
    def _connect_to_mongodb_atlas(atlas_connection_uri: str):
        ...
        client = AsyncIOMotorClient(
            ...
            driver=DriverInfo(
                ...
                version=version("chatgpt_retrieval_plugin"),
            ),
        )
        return client
